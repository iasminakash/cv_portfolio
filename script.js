document
  .querySelector(".hide-footer-button")
  .addEventListener("click", function () {
    document.querySelector("footer").style.display = "none";
  });

document
  .querySelector(".change-light-mode")
  .addEventListener("click", function () {
    const elements = document.querySelectorAll(
      "body, footer, .home, .about, .skills, .resume, .projects, .contact"
    );
    const graph = document.querySelector(
      ".project-contribution-graph-svg",
    );
...
function moveCarousel() {
  ...
  const visibleCards = Math.floor(
    document.querySelector(".carousel-window").offsetWidth / (cardWidth + gap)
  );
